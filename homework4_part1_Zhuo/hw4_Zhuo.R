# require packages : tm, SnowballC

mydata<-read.csv("KSU.csv") # change dir to current R code folder
#mydata<-read.csv("Bowman.csv")
shooting_frame <- as.data.frame(mydata)
library(tm)

myCorpus <- Corpus(VectorSource(shooting_frame$tweet_text))
...
#comebination
myCorpus<- tm_map(myCorpus, gsub, pattern="kent state", replacement="KSU")
myCorpus<- tm_map(myCorpus, gsub, pattern="state", replacement="KSU")
...
# word cloud
wordcloud(d$word, d$freq, min.freq=20,random.color=TRUE, colors=rainbow(7))